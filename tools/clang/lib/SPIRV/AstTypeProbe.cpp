...
QualType desugarType(QualType type, llvm::Optional<bool> *isRowMajor) {
  ...
  if (type->isMatrixType()) {
    // SPIR-V row-major is HLSL col-major and SPIR-V col-major is HLSL
    // row-major.
    bool attrRowMajor = false;
    if (hlsl::HasHLSLMatOrientation(type, &attrRowMajor))
      *isRowMajor = !attrRowMajor;
  }
...
QualType getHLSLMatrixType(ASTContext &astContext, Sema &S, QualType elemType,
  ...
  Expr *colExpr = IntegerLiteral::Create(astContext, llvm::APInt(32, columns),
                                         I32Ty, noLoc);
  return S.BuildMatrixType(elemType, false, S.getLangOpts().HLSLDefaultRowMajor,
                           rowExpr, colExpr, noLoc);
}
